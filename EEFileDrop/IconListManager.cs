...
namespace EEFileDrop
{
    ...
    public class IconListManager
    {
        ...
        public int AddFileIcon(string filePath)
        {
            ...
            if (this._extensionList.ContainsKey(extension.ToUpper()))
            {
                return (int)this._extensionList[extension.ToUpper()]; //return existing index
            }
            // It's not already been added, so add it and record its position.

            int pos = ((ImageList)this._imageLists[0]).Images.Count; //store current count -- new item's index

            if (this.ManageBothSizes)
            {
                //managing two lists, so add it to small first, then large
                ((ImageList)this._imageLists[0]).Images.Add(IconReader.GetFileIcon(filePath, IconReader.IconSize.Small,
                    false));
                ((ImageList)this._imageLists[1]).Images.Add(IconReader.GetFileIcon(filePath, IconReader.IconSize.Large,
                    false));
            }
            else
            {
            ...
            this.AddExtension(extension.ToUpper(), pos); // add to hash table
            return pos;
        }
        ...
        public void ClearLists()
        {
            ...
            this._extensionList.Clear(); //empty hashtable of entries too.
        }
    }
}